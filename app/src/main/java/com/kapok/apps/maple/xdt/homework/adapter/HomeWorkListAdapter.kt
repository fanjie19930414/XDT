...
class HomeWorkListAdapter(
    ...
    dataList: MutableList<HomeWorkListItemBean>,
    private val isTeacher: Boolean
) :
    BaseQuickAdapter<HomeWorkListItemBean, BaseViewHolder>(
        R.layout.item_homework_list_teacher,
        dataList
    ) {

    @SuppressLint("SetTextI18n")
    override fun convert(helper: BaseViewHolder, item: HomeWorkListItemBean) {
        ...
        when {
            item.submitStatus == 1 -> {
                ivHomeWorkParentTag.setImageResource(R.mipmap.label_unsubmit)
            }
            item.submitStatus == 2 -> {
                if (item.replyStatus == 2) {
                    ivHomeWorkParentTag.setImageResource(R.mipmap.label_reply)
                } else {
                    ivHomeWorkParentTag.setImageResource(R.mipmap.label_submitted)
                }
