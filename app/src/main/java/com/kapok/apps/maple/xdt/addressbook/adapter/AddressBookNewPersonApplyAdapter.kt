 ...
 * 申请列表Adapter
 */
class AddressBookNewPersonApplyAdapter(
    val context: Context,
    dataList: MutableList<AddressBookNoHandel>
) : BaseQuickAdapter<AddressBookNoHandel, BaseViewHolder>(R.layout.item_apply_newperson, dataList) {
    ...
    override fun convert(helper: BaseViewHolder, item: AddressBookNoHandel?) {
        ...
        tvAgreeApply.setVisible(true)
        tvRefuseApply.setVisible(false)
        // 同意
        helper.addOnClickListener(R.id.tvAgreeApply)
        // 头像
        if (item != null) {
