...
import com.kapok.apps.maple.xdt.classlist.activity.ClassInfoTeacherActivity
...
import com.kapok.apps.maple.xdt.homework.activity.*
import com.kapok.apps.maple.xdt.homework.bean.HomeWorkListItemBean
import com.kapok.apps.maple.xdt.notice.activity.*
...
import kotlinx.android.synthetic.main.fragment_class_detail_teacher.*
...
class ClassDetailTeacherFragment: BaseMvpFragment<ClassDetailTeacherPresenter>(), ClassDetailTeacherView {
    // 班级Id
    ...
    companion object {
        fun instance(classId: Int): ClassDetailTeacherFragment {
            val myFragment = ClassDetailTeacherFragment()
            val bundle = Bundle()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.fragment_class_detail_teacher, null, false)
    }
    ...
    private fun initView() {
        mPresenter = ClassDetailTeacherPresenter(activity!!)
    ...
    private fun initData() {
        ...
        rvClassDetailTeacher.layoutManager = LinearLayoutManager(activity, LinearLayoutManager.HORIZONTAL, false)
        ...
        rvClassInfoListTeacher.layoutManager = LinearLayoutManager(activity)
        workNoticeListTeacherAdapter = TeacherWorkNoticeListAdapter(activity!!, workNoticeListTeacher)
    ...
    private fun initPop() {
        ...
        parentView = LayoutInflater.from(activity).inflate(R.layout.layout_class_list, null)
    ...
    private fun initListener() {
        // 返回
        tvClassDetailBackTeacher.setOnClickListener {
            // 切换班级列表页
        ...
        ivClassDetailSetting.setOnClickListener {
            // dialog
            classDetailSettingDialog = CustomCancelBottomDialog(activity!!, R.style.BottomDialog)
            classDetailSettingDialog.addItem(
                ...
                View.OnClickListener {
                    ...
                    if (teacherList.size > 0) {
                        ...
                        bottomTeacherDialog =
                            CustomBottomClassListDialog(activity!!, R.style.BottomDialog)
                        ...
                        ToastUtils.showMsg(activity!!, "您的班级没有其他老师\n无法转移权限")
                    }
                })
            classDetailSettingDialog.addItem(
                "班级升学",
        ...
        rlClassDetailInfoTeacher.setOnClickListener {
            val intent = Intent(activity, ClassInfoTeacherActivity::class.java)
        ...
        classIconAdapter.onItemClickListener =
            BaseQuickAdapter.OnItemClickListener { _, _, position ->
                when (position) {
                    0 -> {
                        // 课程表
                        val intent = Intent(activity, TimeTableTeacherActivity::class.java)
                        intent.putExtra("classId",classId)
        ...
        parentView.findViewById<TextView>(R.id.tvClassMoney)
            .setOnClickListener {
                ToastUtils.showMsg(activity!!, "记账")
                pop.dismiss()
            }
    }
    ...
    override fun getClassDetailInfo(bean: ClassDetailInfoBean) {
        ...
        if (bean.avatar != null && bean.avatar.isNotEmpty()) {
            GlideUtils.loadImage(activity!!, bean.avatar, ivClassDetailTeacher)
        } else {
    ...
    override fun changeHeaderTeacher(msg: String) {
        ToastUtils.showMsg(activity!!, msg)
        mPresenter.getClassDetailInfo(classId, searchType, AppPrefsUtils.getInt("userId"))
    }
