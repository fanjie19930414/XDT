...
class ClassInfoTeacherPresenter(context: Context) : BasePresenter<ClassInfoTeacherView>(context) {
    ...
    fun classUpdate(classId: Int, searchType: Int, userId: Int) {
        if (checkNetWork()) {
            ...
            classInfoInstance.getClassInfoTeacher(classId, searchType, userId)
    ...
    // 编辑班级资料接口（班主任）
    fun updateClassInfo(avatar: String, classId: Int, className: String, userId: Int) {
        mView.onShowDialog()
        val updateClassInstance = ClassListInstance()
        updateClassInstance.updateClassInfo(avatar, classId, className, userId)
            .execute(object : BaseObserver<String>(mView) {
                override fun onNext(t: String) {
                    super.onNext(t)
                    mView.updateClassDetail(t)
                }
