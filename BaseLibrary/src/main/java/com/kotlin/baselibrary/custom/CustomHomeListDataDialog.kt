...
import kotlinx.android.synthetic.main.dialog_data_custom.wheel0
import kotlinx.android.synthetic.main.dialog_data_custom.wheel1
import kotlinx.android.synthetic.main.dialog_data_custom.wheel2
import kotlinx.android.synthetic.main.dialog_homework_list_data_custom.*
 ...
 * 底部弹窗工具类
 */
class CustomHomeListDataDialog constructor(context: Context, themeResId: Int) : Dialog(context, themeResId) {

    private lateinit var selectDataListener: SelectDataListener
    private lateinit var selectStartOrEndListener: SelectStartOrEndListener
    ...
    interface SelectStartOrEndListener {
        fun selectStart()
    ...
    fun setOnSelectStartOrEndListener(selctStartOrEndListener: SelectStartOrEndListener) {
        this.selectStartOrEndListener = selctStartOrEndListener
    }
    ...
    fun setTitle(title: String) {
        tv_homework_dialog_title.text = title
    }

    private fun initView() {
        setContentView(R.layout.dialog_homework_list_data_custom)
        setProperty()
        initListener()
    }
    ...
    private fun initWheelView() {
        ...
        wheel0.setOnItemSelectedListener { index ->
            ...
            if (mSelectYear == DateUtils.getYear() && mSelectMonth == DateUtils.getMonth() && mSelectDay == DateUtils.getDay()) {
                tvToday.setVisible(true)
            } else {
                tvToday.setVisible(false)
            }
        }
    ...
    private fun initListener() {
        // 取消
        tvHomeListCancel.setOnClickListener {
            dismiss()
        }
        // 确认
        tvHomeListConfirm.setOnClickListener {
            selectDataListener.selectData(mSelectYear.toString(),mSelectMonth.toString(),mSelectDay.toString())
            dismiss()
        }
        // 开始 / 结束
        rg_homework_list.setOnCheckedChangeListener { group, checkedId ->
            when(checkedId) {
                R.id.bt_homework_start -> {
                    selectStartOrEndListener.selectStart()
                }
                R.id.bt_homework_end -> {
                    selectStartOrEndListener.selectEnd()
                }
            }
        }
    }
