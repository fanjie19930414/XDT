...
public class OssUIDisplayer {
    ...
    public void setOnUpLoadResultListener(UpLoadResult upLoadResult) {
        this.upLoadResult = upLoadResult;
    }


    /* 必须在UI线程中初始化handler */
    public OssUIDisplayer(ImageView imageView, ProgressBar bar, TextView infoView, Activity activity) {
        ...
        handler = new Handler(Looper.getMainLooper()) {
            ...
            public void handleMessage(Message inputMessage) {
                ...
                switch (inputMessage.what) {
                    ...
                    case UPLOAD_OK:
                        // new AlertDialog.Builder(OssUIDisplayer.this.activity).setTitle("上传成功").setMessage("upload to OSS OK!").show();
                        if (upLoadResult != null) {
                            upLoadResult.upLoadOK();
                        }
                        break;
                    case UPLOAD_FAIL:
                        info = (String) inputMessage.obj;
                        new AlertDialog.Builder(OssUIDisplayer.this.activity).setTitle("上传失败").setMessage(info).show();
                        if (upLoadResult != null) {
                            upLoadResult.upLoadFAIL(info);
                        }
                        break;
                    case DOWNLOAD_OK:
                        new AlertDialog.Builder(OssUIDisplayer.this.activity).setTitle("下载成功").setMessage("download from OSS OK!").show();
                        break;
                    case SETTING_OK:
                        new AlertDialog.Builder(OssUIDisplayer.this.activity).setTitle("设置成功").setMessage("设置域名信息成功,现在<选择图片>, 然后上传图片").show();
                        break;
                    case DOWNLOAD_FAIL:
                        info = (String) inputMessage.obj;
                        new AlertDialog.Builder(OssUIDisplayer.this.activity).setTitle("下载失败").setMessage(info).show();
                        break;
                    case UPDATE_PROGRESS:
                        OssUIDisplayer.this.bar.setProgress(inputMessage.arg1);
                        //Log.d("UpdateProgress", String.valueOf(inputMessage.arg1));
                        break;
                    case DISPLAY_IMAGE:
                        Bitmap bm = (Bitmap) inputMessage.obj;
                        OssUIDisplayer.this.imageView.setImageBitmap(bm);
                        break;
                    case DISPLAY_INFO:
                        info = (String) inputMessage.obj;
                        OssUIDisplayer.this.infoView.setText(info);
    ...
    //下载成功，显示对应的图片
    void downloadComplete(Bitmap bm) {
        if (null != bm) {
            displayImage(bm);
        }
    ...
    //显示图像
    private void displayImage(Bitmap bm) {
    ...
    //根据ImageView大小自动缩放图片
    Bitmap autoResizeFromStream(InputStream stream) throws IOException {
